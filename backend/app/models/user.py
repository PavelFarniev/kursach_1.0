...
class User(Base, TimestampMixin):
    ...
    gigachat_credentials_encrypted: Mapped[str | None] = mapped_column(
        Text, nullable=True
    )
    ...
    notes = relationship(
        "CourseNote", back_populates="user", cascade="all, delete-orphan"
    )
    feedback_tickets = relationship(
        "FeedbackTicket", back_populates="user", cascade="all, delete-orphan"
    )
